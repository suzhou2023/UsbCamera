...
void UVCStatusCallback::notifyStatusCallback(JNIEnv *env, uvc_status_class status_class, int event,
                                             int selector, uvc_status_attribute status_attribute,
    ...
    pthread_mutex_lock(&status_mutex);
    {
        if (mStatusCallbackObj) {
            ...
            env->CallVoidMethod(mStatusCallbackObj, istatuscallback_fields.onStatus,
                                (int) status_class, event, selector, (int) status_attribute, buf);
            env->ExceptionClear();
...
void UVCStatusCallback::uvc_status_callback(uvc_status_class status_class, int event, int selector,
                                            uvc_status_attribute status_attribute, void *data,
    ...
    statusCallback->notifyStatusCallback(env, status_class, event, selector, status_attribute, data,
                                         data_len);
